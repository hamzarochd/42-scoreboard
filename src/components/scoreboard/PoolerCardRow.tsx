import { ExternalLink, Calendar, MapPin, Medal, Trophy, Crown, User } from 'lucide-react';
import { Pooler } from '@/types';

interface PoolerCardRowProps {
  pooler: Pooler;
  rank: number;
}

/**
 * Pooler card component in row format with ranking (same size as student cards)
 */
export function PoolerCardRow({ pooler, rank }: PoolerCardRowProps) {
            ...
            <div className="flex items-center space-x-3 flex-1">
              <div className="relative">
                <img
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${pooler.firstName}+${pooler.lastName}&background=E6C642&color=000`;
                  }}
                />
                <div 
                  className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-dark-100 ${getStatusColor(pooler.status.isOnline)}`}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center space-x-2">
              ...
              <div className="min-w-0">
                <p className="text-xs text-dark-500">Pool</p>
                <p className="text-sm font-semibold text-white truncate">{pooler.poolMonth}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-semibold text-white truncate">{pooler.campus}</p>
              </div>
            </div>

            <div className="flex items-center space-x-2">
              <User className="w-4 h-4 text-accent-500 flex-shrink-0" />
              <div className="min-w-0">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-semibold text-white">{pooler.projects.length}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 flex-1 ml-6">
            <div className="relative">
              ...
              />
              <div 
                className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-dark-100 ${getStatusColor(pooler.status.isOnline)}`}
                title={pooler.status.isOnline ? 'Online' : 'Offline'}
              />
            </div>
          ...
          {/* Stats Grid - Adapted for Poolers */}
          <div className="grid grid-cols-2 gap-6 min-w-[280px] ml-8">
            <div className="flex items-center space-x-3">
              <Calendar className="w-5 h-5 text-dark-400" />
              <div>
                <p className="text-xs text-dark-500">Pool Year</p>
                <p className="text-sm font-semibold text-white">{pooler.poolYear}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs text-dark-500">Campus</p>
                <p className="text-sm font-semibold text-white">{pooler.campus}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs text-dark-500">Pool Month</p>
                <p className="text-sm font-semibold text-white">{pooler.poolMonth}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-white">
                  {pooler.status.isOnline ? 'Online' : 'Offline'}
                </p>
              </div>
            </div>
          </div>
